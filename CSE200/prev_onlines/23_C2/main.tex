...
% amsthm theorem-like environment, used later for the weak-field circular orbit proposition
\newtheorem{proposition}{Proposition}

\title{\textsc{THE SOLAR SYSTEM}}
% authblk-style \author[1]{...} with the affiliation inlined via \\ instead of a separate \affil call
...
\maketitle
% \tableofcontents auto-builds from \section/\subsection; \newpage keeps it on its own page
\tableofcontents
\newpage
...
% label={I} is literal here (not a counter format), so both items would print "I" -- kept as in the source article
\begin{enumerate}[label={I}]
    \item \textbf{Inner Planets}
        % $\dagger$ used as a custom symbol bullet instead of the default itemize dot
        \begin{itemize}
...
% two \textcolor runs contrasted in the same sentence
...
% \ref used mid-sentence without "Figure" prefix; \footnote attaches to the sentence, not to \ref itself
...
% \hspace{20px} inserts fixed horizontal space before the trailing condition
\begin{equation}
    \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1, \hspace{20px} a >= b > 0
\end{equation}
Here a and b are the semi-axes.

% nine subfigures grouped into three labeled rows (Star / Inner / Outer) via \textbf{...}\par\medskip before each group
\begin{figure}[H]
    \centering
...
In polar form,
% \label placed on the same line as \begin{equation} works the same as on its own line
...
% $\diamond$ symbol bullet, same trick as $\dagger$ earlier
\begin{itemize}
...
% equation* + split: multi-line derivation with NO equation number at all
\begin{equation*}
\begin{split}
...
\end{equation}
% amsthm proposition, numbered automatically via the \newtheorem in the preamble
...
% multirow for "Planet", multicolumn + cline grouping "Orbital parameters" and "Relativistic notation"
\begin{table}[H]
\centering
...
% unsrtnat orders references by citation order rather than alphabetically
