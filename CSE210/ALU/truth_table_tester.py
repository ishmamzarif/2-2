...
def alu(cs2, cs1, cs0, A, B, Cin=0):
    ...
    # ---------- operation ----------
    if op == (0,0,0):            # SUB WITH BORROW: A - B
        B_comp = (~B_u & 0b1111)
        raw = A_u + B_comp 
        op_type = "SUB WITH BORROW"

    elif op == (0,0,1):          # SUB: A - 1
        B_comp = (~B_u & 0b1111) + 1
        raw = A_u + B_comp 
        op_type = "SUB"

    elif op == (1,0,0):          # ADD + Cin
        raw = A_u + B_u + Cin
        op_type = "ADD"

    elif op == (1,0,1):          # TRANSER
        raw = A_u + (0b1111) + 1
        op_type = "TRANSER"
    ...
    if op_type in ("ADD", "SUB", "SUB WITH BORROW","TRANSER"):
        C = (raw >> 4) & 1  # carry-out
    elif op_type == "SUB":
...
def op_name(cs2, cs1, cs0):
    op = (cs2, cs1, cs0)
    if op == (0,0,0): return "SUB WITH BORROW"
    if op == (0,0,1): return "SUB"
    if op == (1,0,0): return "ADD"
    if op == (1,0,1): return "TRANSFER"
    if op[1:] == (1,0): return "XOR"
    if op[1:] == (1,1): return "AND"
    return "UNKNOWN"


def check_table(filename="my_truth_table.txt", outfile="alu_errors.txt"):
    with open(filename) as f, open(outfile, "w") as out:
        ...
        for line_no, row in enumerate(reader, start=2):
            ...
            Z_expected = int(row[11])
            C_expected = int(row[12])
            V_expected = int(row[13])
            S_expected = int(row[14])
            F_expected = list(map(int, row[16:20]))
