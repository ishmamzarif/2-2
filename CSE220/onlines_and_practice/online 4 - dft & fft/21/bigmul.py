...
from transforms import NTTTransformer
...
# BONUS (NTT). The float engines are bounded by the 53-bit mantissa; the NTT is
# bounded by its prime instead, and that bound is far tighter. Every product
# coefficient must satisfy  n * (B-1)**2 < NTTTransformer.MOD (~9.98e8):
#
#   base 10^4 -> 5000 limbs * 9999**2 = 5.00e11   500x too big
#   base 10^3 -> 6666 limbs *  999**2 = 6.65e9    6.7x too big
#   base 10^2 -> 10000 limbs *  99**2 = 9.80e7    10x of headroom  <-- use this
#
# So exactness costs a smaller base and therefore a longer transform.
NTT_BASE_DIGITS = 2


def base_digits_for(method):
    """Digits per limb for a method: the NTT needs a smaller base than the rest."""
    return NTT_BASE_DIGITS if method == "ntt" else BASE_DIGITS
...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    # removes all leading zeroes to the left
    text = text.lstrip("0")
    ...
    limbs = []
    n = len(text)
    for end in range(n, 0, -base_digits):
...
def multiply_transform(a, b, engine):
    ...
    if engine.name == "arbitrary":
        N = length                  # Bluestein handles any length: no padding
    else:
        N = next_power_of_two(length)

    padded_a = np.zeros(N, dtype=np.float64)
    padded_b = np.zeros(N, dtype=np.float64)
    padded_a[:n] = a
    padded_b[:q] = b

    out = engine.inverse( engine.transform(padded_a) * engine.transform(padded_b) )

    coeffs = np.rint(out.real).astype(np.int64)
    return coeffs[:length], N
...
def multiply_schoolbook(a, b):
    ...
    n = len(a)
    q = len(b)
    out = np.zeros(n + q - 1, dtype=np.int64)
...
def multiply_ntt(a, b):
    """
    BONUS: the same spectral convolution, but exact.

    Identical in shape to multiply_transform -- pad, transform both, multiply
    the spectra pointwise, invert -- except that every value is an integer mod
    a prime. There is no .real and no np.rint, because nothing was ever
    approximate.
    ...
    # The bound that replaces the mantissa argument. A coefficient at or above
    # MOD would wrap and be silently lost, so refuse rather than lie.
    bound = min(len(a), len(b)) * int(max(a.max(), b.max())) ** 2
    if bound >= engine.MOD:
        raise ValueError(
            "NTT overflow: coefficients may reach %d, but the modulus is %d. "
            "Lower NTT_BASE_DIGITS." % (bound, engine.MOD))

    padded_a = np.zeros(N, dtype=np.int64)
    padded_b = np.zeros(N, dtype=np.int64)
    padded_a[:len(a)] = a
    padded_b[:len(b)] = b

    spectrum = engine.transform(padded_a) * engine.transform(padded_b) % engine.MOD
    coeffs = engine.inverse(spectrum)
    return coeffs[:length], N
...
def multiply(text_a, text_b, method):
    ...
    """
    # TODO: implement this function
    base_digits = base_digits_for(method)
    sign_a, a = to_limbs(text_a, base_digits)
    sign_b, b = to_limbs(text_b, base_digits)

    if method == "schoolbook":
        coeffs, N = multiply_schoolbook(a, b), 0
    elif method == "ntt":
        coeffs, N = multiply_ntt(a, b)
    elif method in {"dft", "fft", "arbitrary"}:
        engine = {
            "dft": DFTAnalyzer,
            "fft": FFTTransformer,
            "arbitrary": ArbitraryLengthFFT,
        }[method]()
        coeffs, N = multiply_transform(a, b, engine)
    else:
        raise ValueError("unknown method given")

    return from_limbs(sign_a * sign_b, coeffs, base_digits), N, a, b
...
def run_single(path, method, out_dir):
    ...
    lines = [
        ...
        "base                : 10^%d" % base_digits_for(method),
        "limbs of A / B      : %d / %d" % (len(a), len(b)),
        "transform length N  : %s" % (N if N else "n/a"),
        "digits of product   : %d" % digits_p,
        "verification        : %s" % verdict,
    ]
    write_report(os.path.join(out_dir, "report.txt"), lines)
    print("\n".join(lines))

    if verdict == "MISMATCH":
        raise SystemExit("verification FAILED for %s" % path)   #
