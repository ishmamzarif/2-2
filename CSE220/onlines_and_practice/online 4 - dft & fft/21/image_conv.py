...
def transform_2d(plane, engine):
    ...
    # convolving along the rows
    rows = np.array([engine.transform(row) for row in plane])
    # then using that intermediate result and convolving along the cols
    cols = np.array([engine.transform(col) for col in rows.T])
    return cols.T
...
def inverse_2d(spectrum, engine):
    ...
    rows = np.array([engine.inverse(row) for row in spectrum])
    cols = np.array([engine.inverse(col) for col in rows.T])
    return cols.T
    # raise NotImplementedError("Implement inverse_2d")

def padded_shape(image_shape, kernel_shape, engine):
    # size used for linear convolution
    H, W = image_shape
    kh, kw = kernel_shape
    lin_h, lin_w = H + kh - 1, W + kw - 1      
    # Only radix-2 needs a power of two. The naive DFT runs at any length, and
    # Bluestein transforms at the exact linear size, so both use lin_h x lin_w.
    if engine.name == "fft":
        return next_power_of_two(lin_h), next_power_of_two(lin_w)
    return lin_h, lin_w                         
...
def convolve_plane(plane, kernel, engine, circular=False):
    ...
    if circular == False:
        P, Q = padded_shape(plane.shape, kernel.shape, engine)
    ...
    out = inverse_2d(spectrum, engine).real
...
def convolve_image(image, kernel, engine, circular=False):
    ...
    planes = [convolve_plane(image[:, :, c], kernel, engine, circular)
              for c in range(image.shape[2])]             # R, G, B independently
    return np.stack(planes, axis=2)                       # back to (H, W, 3)
...
def convolve_plane_direct(plane, kernel):
    ...
    for r in range(H):
        for c in range(W):
            total = 0.0
            for i in range(kh):
                rr = r + kh // 2 - i          # the flip: kernel index i reads BACKWARD
                if rr < 0 or rr >= H:
                    continue                   # out of range = zero, so just skip
                for j in range(kw):
                    cc = c + kw // 2 - j
                    if cc < 0 or cc >= W:
                        continue
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    H, W = image.shape[0], image.shape[1]      # works for both (H,W) and (H,W,3)

    if kernel_name == "bokeh":                 # each kernel takes a DIFFERENT keyword
    ...
    engine = {"dft": DFTAnalyzer, "fft": FFTTransformer,
              "arbitrary": ArbitraryLengthFFT}[engine_name]()
    ...
    plane = image if image.ndim == 2 else image[:, :, 0]   # first colour plane if colour
    crop = plane[:64, :64].copy()                          # 64x64 only -- direct is O(N^2 K^2)
    spectral = convolve_plane(crop, kernel, engine, circular=False)
    direct = convolve_plane_direct(crop, kernel)
    error = float(np.max(np.abs(spectral - direct)))
    verdict = "MATCH" if error < 1e-9 else "MISMATCH"

    P, Q = padded_shape((H, W), kernel.shape, engine)
    lines = [
        "Task B -- 2D convolution through the frequency domain",
        "image               : %s  (%d x %d, %s)" % (
            path, H, W, "gray" if image.ndim == 2 else "RGB"),
        "kernel              : %s  (%d x %d)" % (kernel_name, kh, kw),
        "engine              : %s" % engine_name,
        "linear-conv size    : %d x %d" % (H + kh - 1, W + kw - 1),
        "transform size      : %d x %d" % (P, Q),
        "max |spectral - direct| on 64x64 crop : %.3e" % error,
