...
def signal_from_samples(start_time, end_time, samples):
    ...
    for time, val in samples.items():
        signal.set_value_at_time(int(time), val)
...
def impulse_moving_average(length):
    ...
    for n in range(0, length):
        signal.set_value_at_time(n, 1 / length)
...
def max_absolute_difference(first_signal: DiscreteSignal, second_signal: DiscreteSignal):
    ...
    for t in range(start, end + 1):
        mx = max(abs(first_signal.get_value_at_time(t) - second_signal.get_value_at_time(t)), mx)
